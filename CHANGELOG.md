...
- **⚠ PRAZO — Reforma Tributária (IBS/CBS) vence em 03/08/2026.** Situação em 31/07/2026, com 3 dias restantes e as duas ações ainda paradas: (1) **contatar o suporte da Spedy** sobre a API de NF-e, que hoje **não tem campo nenhum de IBS/CBS** (confirmado contra o schema deles em `https://api.spedy.com.br/llms.txt`) — é bloqueio do provedor, não resolve com código nosso; (2) **configurar a reforma no painel da Spedy** para NFS-e, que já suporta: Configurações → Geral → "Habilitar campos da Reforma Tributária", depois Tributação → Regra de Nota de Serviço → NBS + CST da reforma + Código Indicador de Operação + Classificação Tributária. Essa segunda é tarefa de configuração/contador e **dá pra fazer sem esperar programação**. Atinge os **23 clientes de Regime Normal** (Lucro Presumido/Real); Simples Nacional só entra em 2027. No código já existem os campos `ibsCbsCst` e `cClassTrib` em `ProdutoRecord` + `produto-form.tsx`, guardando o dado, mas **nada é transmitido** porque não há payload real pra enviar ainda.
