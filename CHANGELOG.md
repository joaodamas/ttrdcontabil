...
- **Configurar o SMTP de verdade** — código corrigido e commitado (`functions/src/email/secrets.ts`), dono já rodou `firebase functions:secrets:set SMTP_USER`/`SMTP_PASS` (conta `sistema@ttrdcontabil.com.br`, Titan Email) e `functions/.env` foi criado com host/porta. Function redeployada — índice ok, secrets concedidos, mas o teste manual (`gcloud scheduler jobs run`) ainda retornou `Invalid login: 535 5.7.8 Error: authentication failed`. Falta confirmar se a senha salva no secret bate com a senha real da conta (possível erro de digitação) antes de considerar resolvido.
