...
export default async function CompetenciaDetalhePage({
  params,
}: {
  ...
  const competencia = { id: doc.id, ...doc.data() } as Record<string, unknown>

  const [tarefasSnap, lancamentosSnap, nfseSnap] = await Promise.all([
    adminDb
      .collection('tarefas')
  ...
  const tarefas = tarefasSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as Array<Record<string, unknown>>
  const lancamentos = lancamentosSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as Array<Record<string, unknown>>
  const nfse = nfseSnap.docs.map((d) => ({ id: d.id, ...d.data() })) as Array<Record<string, unknown>>
