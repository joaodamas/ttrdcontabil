import { useFiscalReadinessQuery, useFiscalSnapshotNfeQuery, useFiscalSnapshotQuery } from './queries'
...
export function useFiscalDashboardNfe() {
  const query = useFiscalSnapshotNfeQuery()
  return {
    ...query,
    emitidaMesCount: query.data?.emitidaMesCount ?? 0,
    somaEmitidaMes: query.data?.somaEmitidaMes ?? 0,
